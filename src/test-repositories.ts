...
if (recentSessions.length === 3) {
  ...
  console.log(`         - Session IDs: ${recentSessions.map(s => s.session_id.substring(0, 8)).join(', ')}`);
  
  // Note: Since all sessions are created at nearly the same time,
  // the order may vary. The important thing is that we get all 3 sessions.
