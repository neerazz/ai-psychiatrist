import type { ChatFunction, CrisisResult, Message } from '../types.ts';
...
export class CrisisDetectorAgent {
  ...
  /**
   * Fast regex-only check (< 1ms). Used by coordinator to decide
   * whether to run the full AI assessment in the background.
   */
  regexCheck(input: string): RegexAnalysis & { interventionMessage?: string } {
    const result = this.regexAnalyze(input);
    return {
      ...result,
      interventionMessage: result.tier ? INTERVENTION_MESSAGES[result.tier] : undefined,
    };
  }
